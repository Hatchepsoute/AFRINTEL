# Comparaison AFRINTEL : mai et juin 2026

👉🏾 [English version](README.md)

## Comparaison générale

| Indicateur | Mai | Juin | Évolution |
|---|---:|---:|---:|
| Total des incidents | 103 | 40 | -63 (-61,2 %) |
| Ransomware | 17 | 20 | +3 (+17,6 %) |
| Fuites de données et ventes d'accès | 43 | 20 | -23 (-53,5 %) |
| Revendications DDoS | 43 | 0 | -43 |
| Pays distincts après éclatement des fiches multi-pays | 18 | 20 | +2 |

La baisse de juin vient entièrement des fuites de données et ventes d'accès. Le ransomware, lui, va dans l'autre sens, de 17 à 20 fiches.

## Évolution par pays

| Libellé pays direct | Mai | Juin | Évolution |
|---|---:|---:|---:|
| 🇲🇦 Maroc | 51 | 9 | -42 |
| 🇿🇦 Afrique du Sud | 14 | 6 | -8 |
| 🇪🇬 Égypte | 16 | 4 | -12 |
| 🇳🇬 Nigeria | 3 | 4 | +1 |
| 🇹🇳 Tunisie | 5 | 4 | -1 |
| 🇱🇾 Libye | 0 | 3 | +3 |
| 🇩🇿 Algérie | 2 | 0 | -2 |
| 🇹🇿 Tanzanie | 2 | 1 | -1 |
| 🇰🇪 Kenya | 1 | 1 | 0 |
| 🇸🇳 Sénégal | 1 | 1 | 0 |
| Fiches multi-pays | 3 | 2 | -1 |

Les libellés pays directs couvrent 100 fiches en mai, auxquelles s'ajoutent 3 fiches multi-pays, et 38 fiches directes en juin, auxquelles s'ajoutent 2 fiches multi-pays.

## Évolution par secteur

| Secteur | Mai | Juin | Évolution |
|---|---:|---:|---:|
| Government / Administration | 49 | 12 | -8 |
| Finance / Banking | 3 | 6 | +3 |
| Education / University | 3 | 4 | +1 |
| E-commerce / Retail | 4 | 4 | 0 |
| Healthcare / Medical | 1 | 3 | +2 |
| Human resources / Recruitment | 5 | 0 | -5 |
| Industry / Manufacturing / Construction | 5 | 2 | -3 |
| Transport / Aviation | 2 | 2 | 0 |
| Telecommunications | 3 | 0 | -3 |
| NGO / Associations | 2 | 0 | -2 |

Government / Administration reste en tête même en tombant de 20 à 12 fiches. Finance / Banking double, de 3 à 6.

## Évolution des acteurs

Databasehooligan menait mai avec 8 fiches, 404Crew à 5, TheGentlemen à 4. Juin a une tête d'affiche différente : anisanas2 à 7, DeadLock à 4, LockBit 5.0 à 3.

## Évaluation CTI

Juin compte moins de fiches au total, mais couvre en fait davantage de pays une fois les entrées multi-pays éclatées. Le Maroc prend la tête des libellés pays directs pendant que les concentrations en Égypte et en Afrique du Sud reculent. L'ensemble de la répartition penche de nouveau vers le ransomware.

## Priorités SOC

1. Maintenir la préparation au ransomware avec des sauvegardes testées, une couverture EDR et le contrôle des accès privilégiés.
2. Prioriser la surveillance du secteur public et du secteur financier.
3. Détecter les exports anormaux de bases de données, la préparation d'archives et les transferts sortants.
4. Rapprocher les publications répétées par victime, domaine et date afin d'éviter les doubles comptages.

## Conclusion

Juin se solde par 40 incidents contre 103 en mai. Les fuites et ventes d'accès reculent de 21 fiches ; le ransomware en gagne trois.

*AFRINTEL, Open African CTI Monitoring Initiative*
