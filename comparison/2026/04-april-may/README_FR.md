# Comparaison AFRINTEL : avril et mai 2026

👉🏾 [English version](README.md)

## Comparaison générale

| Indicateur | Avril | Mai | Évolution |
|---|---:|---:|---:|
| Total des incidents | 60 | 57 | -3 (-5,0 %) |
| Ransomware | 20 | 16 | -4 (-20,0 %) |
| Fuites de données et ventes d'accès | 40 | 41 | +1 (+2,5 %) |
| Pays distincts après éclatement des fiches multi-pays | 16 | 18 | +2 |

Le volume global baisse légèrement. Les fuites de données et les ventes d'accès restent stables, tandis que les publications ransomware passent de 20 à 16.

## Évolution par pays

| Libellé pays direct | Avril | Mai | Évolution |
|---|---:|---:|---:|
| 🇪🇬 Égypte | 11 | 16 | +5 |
| 🇿🇦 Afrique du Sud | 8 | 14 | +6 |
| 🇲🇦 Maroc | 17 | 7 | -10 |
| 🇹🇳 Tunisie | 4 | 5 | +1 |
| 🇳🇬 Nigeria | 4 | 3 | -1 |
| 🇩🇿 Algérie | 4 | 2 | -2 |
| 🇹🇿 Tanzanie | 0 | 2 | +2 |
| 🇰🇪 Kenya | 2 | 1 | -1 |
| 🇬🇭 Ghana | 2 | 1 | -1 |
| 🇸🇳 Sénégal | 1 | 1 | 0 |
| 🇪🇹 Éthiopie | 1 | 1 | 0 |
| Fiches multi-pays | 1 | 3 | +2 |

Les libellés pays directs représentent 59 fiches en avril, plus une fiche multi-pays, et 54 fiches en mai, plus trois fiches multi-pays.

## Évolution par secteur

| Secteur | Avril | Mai | Évolution |
|---|---:|---:|---:|
| Government / Administration | 15 | 20 | +5 |
| Industry / Manufacturing / Construction | 7 | 5 | -2 |
| Finance / Banking / Insurance | 5 | 3 | -2 |
| Education / University | 8 | 3 | -5 |
| Healthcare / Medical | 4 | 1 | -3 |
| E-commerce / Retail | 4 | 4 | 0 |
| Human resources / Recruitment | 0 | 5 | +5 |
| Telecommunications | 1 | 3 | +2 |

Government / Administration représente 20 des 57 fiches de mai. Human resources / Recruitment apparaît dans 5 fiches de mai, contre aucune en avril.

## Évolution des acteurs

Avril est dominé par Grubder avec 7 fiches, suivi de Payload, APT73/BASHE et TheGentlemen avec 4 chacun. Mai est dominé par Databasehooligan avec 8 fiches, 404Crew avec 5, TheGentlemen avec 4 et NightSpire avec 3.

## Évaluation CTI

Le total mensuel reste proche, mais la distribution évolue. L'Égypte et l'Afrique du Sud remplacent le Maroc comme principales concentrations géographiques. Government / Administration progresse de cinq fiches, tandis que Education / University recule de cinq.

## Priorités SOC

1. Prioriser la surveillance des identités, de la messagerie et des accès privilégiés dans le secteur public.
2. Surveiller les plateformes RH et de recrutement pour détecter les exports, créations de comptes et accès inhabituels.
3. Déclencher des alertes sur les extractions de bases de données, la préparation d'archives et les transferts sortants volumineux.
4. Étendre la surveillance au-delà des principales concentrations nationales, car les fiches multi-pays passent de une à trois.

## Conclusion

Mai compte 57 incidents contre 60 en avril. La baisse provient du ransomware, tandis que les fuites et ventes d'accès progressent d'une fiche.

*AFRINTEL, Open African CTI Monitoring Initiative*
