...
## 📊 Volume de données exfiltrées par pays (connues)

> NB : ce graphe ne prend en compte **que** les volumes explicitement mentionnés dans les listings.

```mermaid
xychart-beta
  title "Volumes exfiltrés (TB) — Février 2026 (connus)"
  x-axis ["Senegal","Somalia","Ghana","Tanzania","South Africa"]
  y-axis "TB" 0 --> 140
  bar [139, 3.5, 2.5, 0.85, 0.15179]
```

## 🧩 Cartographie Acteurs → Victimes (verticale)

```mermaid
flowchart TD

  subgraph A1["The Green Blood Group"]
    V1["DAF Sénégal 🇸🇳 (139 To)"]
  end

  subgraph A2["0APT"]
    V2["BlueSky Aviation 🇸🇴 (3,5 To)"]
    V3["Global Media Alliance 🇬🇭 (2,5 To)"]
    V4["Vertex Law Chambers 🇹🇿 (850 Go)"]
  end

  subgraph A3["thegentlemen"]
    V5["Wells Fargo 🇰🇪"]
    V6["Ghana Bauxite 🇬🇭"]
    V7["Nile Air 🇪🇬"]
    V8["Municipalité d’Intsika Yethu 🇿🇦"]
    V9["BITS 🇹🇳"]
  end

  subgraph A4["lockbit5"]
    V10["Sands Suites 🇲🇺"]
    V11["Ministère de l’Agriculture 🇪🇬"]
    V12["Diesel-Electric 🇿🇦"]
  end

  subgraph A5["incransom"]
  ...
  subgraph A9["vect"]
    V18["EnerTec 🇿🇦 (151,79 Go)"]
  end

  subgraph A10["apt73/bashe"]
    V19["amtaar.com 🇸🇩"]
  end

  subgraph A11["killsec"]
    V20["Getly 🇳🇬"]
  end
```

## 📊 Diagramme Victimes par Pays (PNG)

![Victimes par pays](afrintel_feb2026_victims_by_country.png)
